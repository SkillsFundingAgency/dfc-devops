...
foreach ($collection in $Collections.GetEnumerator()) {
    ...
    $parameters = "/s:JsonFile /s.Files:blobs://$StorageKey@$($ContainerUrl.Replace('https://', ''))/$($collection.Value)-$($collection.Key)-backup.json /t:DocumentDBBulk /t.PartitionKey:/PartitionKey /t.ConnectionString:AccountEndpoint=https://$CosmosAccountName.documents.azure.com:443/;AccountKey=$CosmosKey;Database=$Database /t.Collection:$($Collection.Key)" 
