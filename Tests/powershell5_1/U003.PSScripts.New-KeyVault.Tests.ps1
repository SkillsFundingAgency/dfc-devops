...
Describe "New-KeyVault unit tests" -Tag "Unit" {
    ...
    It "Should create a key vault if one does not exist" -Foreach @{rgname = $rgname; kvname =$kvname }{
    ...
    It "Should not create anything if the key vault already exist" -Foreach @{rgname = $rgname; kvname =$kvname }{
